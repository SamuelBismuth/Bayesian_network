...
public class Network {
	...
	public Factors create_factors(
			...
			List<Probability> evidence) {
		for(Variable variable : get_factors_variable) 
			factors.add(create_factor(variable, evidence));
	...
	private Factor create_factor(
		...
		for (Cond_prob cp : variable.getC_p()) {
			c_p.add(new Cond_prob(cp.deleteEvidence(evidence)));
		}
		boolean flag = true;
		for (Cond_prob cp : variable.getC_p()) {
			for(Condition condition : cp.getProbability().keySet()) {
				for (Probability probability : condition.getJoin_probability()) {
					List<Probability> list_prob = evidence.stream().filter(item->probability.
							getVariable_name()
	 ...
	 */
	public List<Variable> delete_irrelevant(List<Variable> get_all_variable) {
		Set<Variable> relevent_variable = new HashSet<>(get_all_variable);
		for(Variable variable : get_all_variable) 
			this.get_ancestor(relevent_variable, variable);
		List<Variable> all_variable = new ArrayList<>(this.getVariables());
		all_variable.removeAll(relevent_variable);
	...
	private void get_ancestor(Set<Variable> ancestors, Variable variable) {
		if(variable.getParents() != null) {
			ancestors.addAll(this.find_variables_by_names(variable.getParents()));
			for (Variable parent : this.find_variables_by_names(variable.getParents()))
